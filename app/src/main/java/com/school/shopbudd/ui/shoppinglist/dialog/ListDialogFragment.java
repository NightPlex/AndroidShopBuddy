...
import com.school.shopbudd.ui.shoppinglist.adapter.ListDialogFocusListener;
import com.school.shopbudd.ui.shoppinglist.reminder.ReminderReceiver;
import com.school.shopbudd.ui.shoppinglist.reminder.ReminderScheduler;
...
public class ListDialogFragment extends DialogFragment {
    ...
    public static ListDialogFragment newEditInstance(ListItem item, ActivityCache cache) {
        editDialog = true;
        ListDialogFragment dialogFragment = getListDialogFragment(item, cache);
        return dialogFragment;
    }

    public static ListDialogFragment newAddInstance(ListItem item, ActivityCache cache) {
        editDialog = false;
        ListDialogFragment dialogFragment = getListDialogFragment(item, cache);
        return dialogFragment;
    }

    private static ListDialogFragment getListDialogFragment(ListItem item, ActivityCache cache) {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        dialogCache.getStatisticsSwitch().setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (dialogCache.getStatisticsSwitch().isChecked()) {
                    MessageUtil.showToast(getActivity(), R.string.pref_statistics_toast_on, Toast.LENGTH_LONG);
                } else {
                    MessageUtil.showToast(getActivity(), R.string.pref_statistics_toast_off, Toast.LENGTH_SHORT);
                }
            }
        });
        ...
        if (editDialog) {
            ...
            boolean statisticsEnabled = sharedPreferences.getBoolean(SettingConsts.STATISTICS_ENABLED, false);
            dialogCache.getStatisticsSwitch().setChecked(statisticsEnabled);
        }

        if (!LineUtil.isEmpty(item.getDeadlineDate())) {
        ...
        reminderSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (reminderSwitch.isChecked()) {
                    if (!PreferenceManager.getDefaultSharedPreferences(getContext()).getBoolean(SettingConsts.NOTIFICATIONS_ENABLED, true)) {
                        MessageUtil.showToast(getContext(), R.string.notification_setting_status, Toast.LENGTH_LONG);
                    }
                    dialogCache.getReminderLayout().setVisibility(View.VISIBLE);
                } else {
                    dialogCache.getReminderLayout().setVisibility(View.GONE);
        ...
        dialogCache.getListNameText().setOnFocusChangeListener(new ListDialogFocusListener(dialogCache));
        dialogCache.getListNotes().setOnFocusChangeListener(new ListDialogFocusListener(dialogCache));
                ...
                shoppingListService.saveOrUpdate(item)
                        .doOnCompleted(() ->
                        {
                            ...
                            if (reminderSwitch.isChecked()) {
                                ...
                                Intent intent = new Intent(cache.getActivity(), ReminderScheduler.class);
                                intent.putExtra(ReminderScheduler.MESSAGE_TEXT, message);
                            ...
                            if (!editDialog) {
                                // go to new list
                                Intent intent = new Intent(cache.getActivity(), ProductActivity.class);
