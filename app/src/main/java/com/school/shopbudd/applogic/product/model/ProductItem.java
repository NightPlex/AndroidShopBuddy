 ...
 * @author NightPlex
 */
public class ProductItem extends ItemAbstract {
    ...
    public String getSummary(Context context) {
        ...
        if (!LineUtil.isEmpty(getProductStore())) {
            sb.append(storeLabel).append(LineUtil.SPACE).append(getProductStore());
            withStore = true;
        }
        if (!LineUtil.isEmpty(getProductCategory())) {
            if (withStore) {
                sb.append(LineUtil.COMMA).append(LineUtil.SPACE);
            }
            sb.append(categoryLabel).append(LineUtil.SPACE).append(getProductCategory());
        }
        return sb.toString();
    }
    ...
    public String getDetailInfo(Context context) {
        ...
        StringBuilder sb = new StringBuilder();
        if (!LineUtil.isEmpty(quantity)) {
            sb.append(quantityDetail);
            sb.append(LineUtil.NEW_LINE);
        }
        if (!LineUtil.isEmpty(productPrice)) {
            sb.append(priceDetail);
            sb.append(LineUtil.NEW_LINE);
        }
        if (!LineUtil.isEmpty(totalProductPrice)) {
            sb.append(totalPriceDetail);
        }

        boolean categoryExists = !LineUtil.isEmpty(productCategory);
        boolean storeExists = !LineUtil.isEmpty(productStore);
        boolean notesExists = !LineUtil.isEmpty(productNotes);
